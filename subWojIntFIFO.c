/*
 * subWojIntFIFO.c
 * The Subramani-Wojciechowski integral UTVPI system solver, with FIFO linear modification
 *
    ...
    if( vertex->examinationCount[Dtype] <= 2 * system->n ){
      ...
      case WHITE:
        ...
        while( edge != NULL ){
          if( vertex->D[WHITE] + edge->weight < edge->head->D[GRAY_REVERSE] ){
            freeQueue(&queue);
            return backtrack(system, vertex, WHITE, edge);
          }
          edge = edge->next;
        }
        edge = vertex->first[GRAY_FORWARD];
      ...
      case BLACK:
        ...
        while( edge != NULL ){
          if( vertex->D[BLACK] + edge->weight < edge->head->D[GRAY_FORWARD] ){
            freeQueue(&queue);
            return backtrack(system, vertex, BLACK, edge);
          }
          edge = edge->next;
        }
        edge = vertex->first[GRAY_REVERSE];
      ...
      case GRAY_FORWARD:
        ...
        while( edge != NULL ){
          if( vertex->D[GRAY_FORWARD] + edge->weight < edge->head->D[BLACK] ){
            freeQueue(&queue);
            return backtrack(system, vertex, GRAY_FORWARD, edge);
          }
          edge = edge->next;
        }
        edge = vertex->first[GRAY_FORWARD];
      ...
      case GRAY_REVERSE:
        ...
        while( edge != NULL ){
          if( vertex->D[GRAY_REVERSE] + edge->weight < edge->head->D[WHITE] ){
            freeQueue(&queue);
            return backtrack(system, vertex, GRAY_REVERSE, edge);
          }
          edge = edge->next;
        }
        edge = vertex->first[GRAY_REVERSE];
