/*
 * lahiriFIFOtrace.c
 * The Lahiri-Musuvathi integral UTVPI system solver, with FIFO linear modification and intermittent predecessor structure traces
 ...
 * traceNumber - number of the trace that crosses this Vertex, in backtrack()
 * inQueue - whether or not this Vertex is currently present within the FIFO queue used by linear()
...
/*
 * Queue is a simple FIFO queue, used to store pointers to Vertices to be examined in linear().
 *
 * newest - pointer to the most-recently-added member of the Queue
 * oldest - pointer to the earliest-added member of the Queue
 */
struct Queue {
  QueueNode * newest;
  QueueNode * oldest;
 ...
 *
 * vertex - pointer to a vertex to be examined in linear()
 * newer - pointer to the QueueNode added to the Queue immediately after this one
 */
struct QueueNode {
  Vertex * vertex;
  QueueNode * newer;
...
static void setSystemForLinear(System * system);
...
int main(int argc, char * argv[]){
  ...
  if( negativeCycle != NULL ){
    ...
    int traceNumber = edge->tail->traceNumber;
    while( edge->tail->traceNumber == traceNumber ){
      fputEdge( edge, output );
      edge->tail->traceNumber = 0;
      edge = edge->tail->L;
    }
  }
  else{
...
/*
 * setSystemForLinear() initializes Vertex struct anonymous union data members only used during the process of generating a linear
 * solution, for all Vertices.
 *
...
static void setSystemForLinear(System * system){
  for(VertexSign i = POSITIVE; i <= NEGATIVE; i++){
    for(int j = 0; j < system->n; j++){
      ...
      system->graph[i][j].examinationCount = 0;
      system->graph[i][j].traceNumber = 0;
      system->graph[i][j].inQueue = false;
    }
  }
}
  ...
  while( vertex != NULL ){
    ...
    if( vertex->examinationCount < 2 * system->n ){
      ...
      if( vertex->examinationCount == tracePoint ){
        ...
        Edge * negativeCycle = traceSystem( system );
        if( negativeCycle != NULL ){
          freeQueue( &queue );
...
static Edge * traceSystem(System * system){
  ...
  for(VertexSign i = POSITIVE; i <= NEGATIVE; i++){
    for(int j = 0; j < system->n; j++){
      if( system->graph[i][j].traceNumber == 0 ){
        system->graph[i][j].traceNumber = traceNumber;
        Edge * negativeCycle = backtrack( system->graph[i][j].L, traceNumber );
        if( negativeCycle != NULL ){
          return negativeCycle;
        }
        traceNumber++;
 ...
 */
static Edge * backtrack(Edge * edge, int traceNumber){
  while( edge != NULL && edge->tail->traceNumber == 0 ){
    edge->tail->traceNumber = traceNumber;
    edge = edge->tail->L;
  }
  if( edge != NULL && edge->tail->traceNumber == traceNumber ){
    return edge;
  }
  return NULL;
}
...
/*
 * queueOffer() adds a pointer to a Vertex to the input Queue, if that Vertex is not already represented within the Queue.
 *
 * queue - pointer to a Queue to add a pointer to a Vertex to
...
/*
 * queuePoll() removes the oldest member of the Queue and returns the pointer to a Vertex stored within that member.
 *
 * queue - pointer to a Queue to remove a pointer to a Vertex from
 */
static Vertex * queuePoll(Queue * queue){
  Vertex * output;
  if( queue->oldest != NULL ){
    ...
    free( oldQN );
    output->examinationCount++;
    output->inQueue = false;
  }
  else {
    output = NULL;
  }
...
/*
 * freeQueue() frees the input Queue
 *
