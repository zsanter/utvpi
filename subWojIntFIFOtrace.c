/*
 * subWojIntFIFOtrace.c
 * The Subramani-Wojciechowski integral UTVPI system solver, with FIFO linear modification, and intermittent predecessor structure
 * traces
 *
...
struct Vertex {
  ...
  int examinationCount[EDGE_TYPE_COUNT];
  bool inQueue[EDGE_TYPE_COUNT];
  Edge * E[BACKTRACKING_INDEX_COUNT];
  int traceNumber[BACKTRACKING_INDEX_COUNT];
  half_int a;
...
static void queueOffer(Queue * queue, Vertex * vertex, EdgeType Dtype);
static void queuePoll(Queue * queue, Vertex ** vertexOut, EdgeType * DtypeOut);
...
static void initializeSystem(void * object, int n, Parser * parser){
  ...
  system->graph[0].traceNumber[NEG_ONE] = 0;
  system->graph[0].traceNumber[POS_ONE] = 0;
  system->graph[0].a = 0;
  ...
  for(EdgeType i = WHITE; i <= GRAY_REVERSE; i++){
    ...
    system->graph[0].examinationCount[i] = 0;
    system->graph[0].inQueue[i] = false;
  ...
  for(int i = 1; i < system->vertexCount; i++){
    ...
    for(EdgeType j = WHITE; j <= GRAY_REVERSE; j++){
      ...
      system->graph[i].examinationCount[j] = 0;
      system->graph[i].inQueue[j] = false;
...
/*
 * relaxNetwork() implements RELAX-NETWORK(). The function returns true for a linearly feasible system. False otherwise. If a 
 *   negative cost cycle is detected, it is placed in system->infeasibilityProof.
 * system - pointer to the System struct storing the overall graph representation
 */
static bool relaxNetwork(System * system){
  ...
  while( vertex != NULL ){
    Edge * edge;
    if( vertex->examinationCount[Dtype] <= 2 * system->n ){

      switch( Dtype ){
      case WHITE:
        edge = vertex->first[BLACK];
        while( edge != NULL ){
          if( vertex->D[WHITE] + edge->weight < edge->head->D[GRAY_REVERSE] ){
            edge->head->D[GRAY_REVERSE] = vertex->D[WHITE] + edge->weight;
            edge->head->L[GRAY_REVERSE] = edge;
            queueOffer(&queue, edge->head, GRAY_REVERSE);
          }
          edge = edge->next;
        }
        edge = vertex->first[GRAY_FORWARD];
        while( edge != NULL ){
          if( vertex->D[WHITE] + edge->weight < edge->head->D[WHITE] ){
            edge->head->D[WHITE] = vertex->D[WHITE] + edge->weight;
            edge->head->L[WHITE] = edge;
            queueOffer(&queue, edge->head, WHITE);
          }
          edge = edge->next;
        }
        break;
      case BLACK:
        edge = vertex->first[WHITE];
        while( edge != NULL ){
          if( vertex->D[BLACK] + edge->weight < edge->head->D[GRAY_FORWARD] ){
            edge->head->D[GRAY_FORWARD] = vertex->D[BLACK] + edge->weight;
            edge->head->L[GRAY_FORWARD] = edge;
            queueOffer(&queue, edge->head, GRAY_FORWARD);
          }
          edge = edge->next;
        }
        edge = vertex->first[GRAY_REVERSE];
        while( edge != NULL ){
          if( vertex->D[BLACK] + edge->weight < edge->head->D[BLACK] ){
            edge->head->D[BLACK] = vertex->D[BLACK] + edge->weight;
            edge->head->L[BLACK] = edge;
            queueOffer(&queue, edge->head, BLACK);
          }
          edge = edge->next;
        }
        break;
      case GRAY_FORWARD:
        edge = vertex->first[BLACK];
        while( edge != NULL ){
          if( vertex->D[GRAY_FORWARD] + edge->weight < edge->head->D[BLACK] ){
            edge->head->D[BLACK] = vertex->D[GRAY_FORWARD] + edge->weight;
            edge->head->L[BLACK] = edge;
            queueOffer(&queue, edge->head, BLACK);
          }
          edge = edge->next;
        ...
        edge = vertex->first[GRAY_FORWARD];
        while( edge != NULL ){
          if( vertex->D[GRAY_FORWARD] + edge->weight < edge->head->D[GRAY_FORWARD] ){
            edge->head->D[GRAY_FORWARD] = vertex->D[GRAY_FORWARD] + edge->weight;
            edge->head->L[GRAY_FORWARD] = edge;
            queueOffer(&queue, edge->head, GRAY_FORWARD);
          }
          edge = edge->next;
      ...
      case GRAY_REVERSE:
        edge = vertex->first[WHITE];
        while( edge != NULL ){
          if( vertex->D[GRAY_REVERSE] + edge->weight < edge->head->D[WHITE] ){
            edge->head->D[WHITE] = vertex->D[GRAY_REVERSE] + edge->weight;
            edge->head->L[WHITE] = edge;
            queueOffer(&queue, edge->head, WHITE);
          }
          edge = edge->next;
        ...
        edge = vertex->first[GRAY_REVERSE];
        while( edge != NULL ){
          if( vertex->D[GRAY_REVERSE] + edge->weight < edge->head->D[GRAY_REVERSE] ){
            edge->head->D[GRAY_REVERSE] = vertex->D[GRAY_REVERSE] + edge->weight;
            edge->head->L[GRAY_REVERSE] = edge;
            queueOffer(&queue, edge->head, GRAY_REVERSE);
          }
          edge = edge->next;
      ...
      case WHITE:
        ...
        while( edge != NULL ){
          if( vertex->D[WHITE] + edge->weight < edge->head->D[GRAY_REVERSE] ){
            freeQueue(&queue);
            return backtrack(system, vertex, WHITE, edge, -1);
          }
          edge = edge->next;
        }
        edge = vertex->first[GRAY_FORWARD];
      ...
      case BLACK:
        ...
        while( edge != NULL ){
          if( vertex->D[BLACK] + edge->weight < edge->head->D[GRAY_FORWARD] ){
            freeQueue(&queue);
            return backtrack(system, vertex, BLACK, edge, -1);
          }
          edge = edge->next;
        }
        edge = vertex->first[GRAY_REVERSE];
      ...
      case GRAY_FORWARD:
        ...
        while( edge != NULL ){
          if( vertex->D[GRAY_FORWARD] + edge->weight < edge->head->D[BLACK] ){
            freeQueue(&queue);
            return backtrack(system, vertex, GRAY_FORWARD, edge, -1);
          }
          edge = edge->next;
        }
        edge = vertex->first[GRAY_FORWARD];
      ...
      case GRAY_REVERSE:
        ...
        while( edge != NULL ){
          if( vertex->D[GRAY_REVERSE] + edge->weight < edge->head->D[WHITE] ){
            freeQueue(&queue);
            return backtrack(system, vertex, GRAY_REVERSE, edge, -1);
          }
          edge = edge->next;
        }
        edge = vertex->first[GRAY_REVERSE];
...
static bool traceSystem(System * system){
  ...
  for(int i = 0; i < system->vertexCount; i++){
    ...
    if( system->graph[i].traceNumber[NEG_ONE] == 0 ){
      if( system->graph[i].L[BLACK] != NULL ){
        bool noNegCycle = backtrack(system, &system->graph[i], BLACK, system->graph[i].L[BLACK], traceNumber);
        if( !noNegCycle ){
          return false;
        }
        traceNumber++;
...
/*
 * backtrack() implements BACKTRACK(). This function fills system->infeasibilityProof with a negative cost cycle and returns 
 *   false, indicating a linearly infeasible system.
 * system - pointer to the System struct storing the overall graph representation
 * x_i - pointer to the initial Vertex
 * t - initial path type
 * e - pointer to the initial edge
 * traceNumber - number left along the predecessor path in order to detect negative cost cycles; must not equal 0
 */
static bool backtrack(System * system, Vertex * x_i, EdgeType t, Edge * e, int traceNumber){
  Vertex * x_c = x_i;
  Edge * e_c = e;
  ...
  while( x_c->E[a_c] == NULL && x_c->L[t_c] != NULL ){
    x_c->E[a_c] = e_c;
    x_c->traceNumber[a_c] = traceNumber;
    e_c = x_c->L[t_c];
    switch(t_c){
    case WHITE:
      switch(e_c->type){
...
static void queueOffer(Queue * queue, Vertex * vertex, EdgeType Dtype){
  if( !vertex->inQueue[Dtype] ){
    QueueNode * newQN = (QueueNode *) malloc( sizeof(QueueNode) );
    newQN->vertex = vertex;
    newQN->Dtype = Dtype;
    newQN->newer = NULL;
    if( queue->newest != NULL ){
      queue->newest->newer = newQN;
    }
    queue->newest = newQN;
    if( queue->oldest == NULL ){
      queue->oldest = newQN;
    }
    vertex->inQueue[Dtype] = true;
  }
}

static void queuePoll(Queue * queue, Vertex ** vertexOut, EdgeType * DtypeOut){
  if( queue->oldest != NULL ){
    QueueNode * oldQN = queue->oldest;
    *vertexOut = oldQN->vertex;
    *DtypeOut = oldQN->Dtype;
    queue->oldest = oldQN->newer;
    if( queue->oldest == NULL ){
      queue->newest = NULL;
    }
    free( oldQN );
    (*vertexOut)->examinationCount[*DtypeOut]++;
    (*vertexOut)->inQueue[*DtypeOut] = false;
  }
  else {
    *vertexOut = NULL;
  }
