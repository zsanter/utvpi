...
/*
 * The System struct contains references to all dynamically-allocated data structures used within multiple functions, along with
 ...
 *   linked list is pointed to by additionsFirst, so that these Edges may be properly freed when no longer needed
 ...
 * - sorts all edge lists by increasing head vertex index
 ...
 * Dtype - distance label type to be added to the Queue
 */
static void queueOffer(Queue * queue, Vertex * vertex, EdgeType Dtype){
...
/*
 * freeIntegerTree() frees the input IntegerTree. Also sets integerTreeVertex in each associated graph Vertex to NULL.
...
/*
 * freeSystem() frees the graph representation stored within system, along with system->additionsFirst, system->
 *   infeasibilityProof, and system->T, if those have yet to be freed.
